...
> **Status:** v0.1.0 is a runnable read-only slice. It supports Zotero navigation, metadata lookup, Markdown-first/PDF-fallback reading, lexical finding, and read-only Markdown migration audits. Semantic indexing and every Zotero mutation remain unimplemented until the authenticated Extension is live-validated.
...
- Local, read-only behavior in v0.1.0; no telemetry, cloud LLM, implicit conversion, or Zotero mutation.
...
```text
zotero-extension/build/zotero-agent-library-0.1.0.xpi
```
