...
The Agentibility Console is a read-only Item-pane section. It displays the child process lifecycle and heartbeat, queued and active Items, coverage counts, recent errors, and selected Item state. Heartbeats continue while extraction or embedding runs. The worker exits on its command pipe's EOF or shutdown command; the Extension also terminates an unresponsive child during shutdown. Disabling the Extension ends automatic indexing, while the derived index and durable queue remain on disk.
