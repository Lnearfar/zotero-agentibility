...
- Foreground retrieval uses the existing index immediately. Changed Literature Items enter a durable refresh queue and Zotero's child worker performs scoped updates. Agent requests begin with retrieval; worker startup reconciliation runs independently in the background.
- Indexing reads current My Library metadata through the authenticated Extension `index_catalog` operation and reads local attachment files. Native Zotero reads include newly added Items immediately. Terminal navigation still uses a read-only SQLite snapshot. Zotero Desktop must be running.
...
- `za-cli index worker --once` consumes one bounded queue batch for diagnosis and exits nonzero on Item failures. The Extension owns `za-cli index worker --managed`, supplies its command pipe, and displays status events in the Agentibility Console.
- `za-cli index status` reads persisted readiness, freshness, coverage, worker state, and queue backlog without traversing Chroma. `app doctor` reports `DEGRADED` when the worker is stopped. Legacy state may report `stats_stale` until the next update or explicit `index status --deep` reconciliation.
...
- `--filters JSON` preserves generic Chroma metadata filtering. Semantic search has no hidden navigation scope.

A successful CLI-originated Full Text mutation durably queues the affected Literature Item. If enqueueing fails, the Zotero mutation remains committed and is reported with an index warning. Native Zotero notifications queue changed parents and attachment parents; periodic reconciliation covers externally edited linked-file contents. Use explicit `index update` or `index reconcile` for unknown bulk changes or operator recovery. Indexes created before the inventory format receive one full compatibility pass; subsequent unchanged updates compare native metadata inventories and local file statistics.
...
One non-blocking `worker.lock` permits a single worker. `update.lock` still serializes Chroma writes against explicit synchronous updates. The worker deletes and directory-syncs only snapshot events for successful Item Keys; failed keys, lock contention, or process termination leave events pending for retry. Failed Item events move to `queue/failed/` for delayed retry; new notifications take priority. Malformed events are quarantined there and are never processed as Items. Queue Item counts are deduplicated. Reprocessing after a crash is safe because source signatures make unchanged scoped updates idempotent.

Root `index-runtime.json` configures heartbeat, retry, reconciliation, debounce, restart, and shutdown intervals and status size limits. It is packaged with the Extension; the Extension passes worker settings through explicit CLI arguments rather than environment overrides. The Extension starts the managed worker with Zotero and supervises unexpected exits. Native notifications supply exact dirty Item Keys through a command pipe. The worker retains the durable queue, processes scoped updates, and reconciles on startup and every 12 hours. Its native catalog read uses the same Zotero process that owns current library state, so committed changes remain visible during SQLite locking and checkpoint windows.

A synchronous `index update` leaves queued events for the worker to acknowledge after rechecking their current sources. Events arriving during an update remain pending. Per-Item failures remain retryable while other Items progress. Search's `refreshing` flag describes active processing; `possibly_stale: true` reflects asynchronous maintenance.

The Agentibility Console is a read-only Item-pane section. It leads with the selected Item's search status (`PDF indexed`, `MD indexed`, `Indexing`, `Queued`, `Failed`, or `Not indexed`), searchable passage count, and partial coverage. Existing indexed content remains searchable during refresh or a failed retry. Canonical Markdown is indexed instead of PDF, not as a second parallel index. Library worker, queue, error counts and affected keys are displayed separately below; a library error need not belong to the selected Item. Between the library summary and error details, a Queued items list shows every pending Item's title, copyable key, indexing/waiting/retry state, and any failure with a suggested corrective action. Successful processing removes an Item from the queue; failed Items remain for automatic retry without blocking other papers. Console text is selectable: copy an Item Key to locate it manually. Each listed error includes its message, not just its code. Heartbeats continue while extraction or embedding runs. The worker exits on its command pipe's EOF or shutdown command; the Extension also terminates an unresponsive child during shutdown. Disabling the Extension ends automatic indexing, while the derived index and durable queue remain on disk.
