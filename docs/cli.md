...
This document defines the target interface. The implemented v0.1.1 read-only slice currently provides `session`, `app`, `pwd`, `cd`, `ls`, `lookup`, `source`, `read`, `find`, and `fulltext audit`; later commands below are not yet available. The installed command reference is `zotero-cli --help`, with `zotero-cli COMMAND --help` or nested group help for arguments and options.
