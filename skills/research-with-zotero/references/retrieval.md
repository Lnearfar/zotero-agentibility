...
1. Search immediately:
   ...
   Search covers My Library and returns cached index freshness metadata. `possibly_stale` is provenance, not a reason to block ordinary retrieval. Use `--collection PATH` for an explicit recursive Collection scope or `--item ITEM_KEY` for several matching Passages within one paper.
